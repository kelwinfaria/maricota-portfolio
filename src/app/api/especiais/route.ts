...
export async function PUT(req: NextRequest) {
  ...
  // delete + insert para persistir a coluna `cover` (a RPC antiga não a conhece).
  const { error: deleteError } = await supabaseAdmin.from('especiais_slots').delete().neq('id', 0)
  if (deleteError) return NextResponse.json({ error: deleteError.message }, { status: 500 })

  const rows = slots.map((s, i) => ({ position: i, type: s.type, ref_id: s.ref_id, label: s.label, cover: s.cover }))
