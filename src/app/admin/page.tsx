        ...
        <nav className="sb" ref={sbRef as React.RefObject<HTMLElement>} id="sb">
          ...
          <div className="sb-nav">
            ...
            {(['dash', 'produtos', 'colecoes'] as const).map(pg => (
              <button key={pg} className={`sbl${page === pg ? ' on' : ''}`} onClick={() => setPage(pg)}>
            ...
            <button className={`sbl${page === 'aparencia' ? ' on' : ''}`} onClick={() => setPage('aparencia')}><span className="ico">🎨</span><span className="lbl">Aparência</span></button>
            <div className="sb-grp">Sistema</div>
            <button className={`sbl${page === 'settings' ? ' on' : ''}`} onClick={() => setPage('settings')}><span className="ico">⚙️</span><span className="lbl">Configurações</span></button>
